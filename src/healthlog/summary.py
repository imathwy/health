...
from .store import CORE_NUTRIENTS, average_ranges, confidence_counts
...
def make_summary(
    *,
    ...
    trends = {
        nutrient: interval_trend(rows, nutrient, start)
...
def render_markdown(summary: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## 每日记录", "", "| 日期 | 日型 | 热量 | 蛋白质 | 照片覆盖 | 置信度 |", "|---|---|---:|---:|---|---|"])
    ...
    if sources:
        lines.extend(f"- `{source}`：{count} 个食物条目" for source, count in sources.items())
    else:
        lines.append("- 尚无食物条目")
    ...
    lines.extend(["", "## 覆盖与限制", ""])
    lines.append(
        f"- 缺失 {len(missing)} 天：{', '.join(missing) if missing else '无'}"
    )
...
def render_html(summary: dict[str, Any]) -> str:
    ...
    for day in summary["days"]:
        kcal = day["nutrients"].get("kcal")
        protein = day["nutrients"].get("protein_g")
        kcal_text = display_interval(kcal["low"], kcal["high"], kcal["unit"]) if kcal else "—"
        protein_text = display_interval(protein["low"], protein["high"], protein["unit"]) if protein else "—"
        day_rows.append(
            "<tr>"
